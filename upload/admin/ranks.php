...
($hook = get_hook('ark_start')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
...
if (isset($_POST['add_rank']))
{
	...
	($hook = get_hook('ark_add_rank_form_submitted')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
	...
	($hook = get_hook('ark_qr_check_rank_collision')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
	...
	($hook = get_hook('ark_qr_add_rank')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
...
else if (isset($_POST['update']))
{
	...
	($hook = get_hook('ark_update_form_submitted')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
	...
	($hook = get_hook('ark_qr_check_rank_collision2')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
	$result = $forum_db->query_build($query) or error(__FILE__, __LINE__);
	if ($forum_db->num_rows($result))
		message(sprintf($lang_admin_ranks['Min posts occupied message'], $min_posts));
	...
	($hook = get_hook('ark_qr_update_rank')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
	$forum_db->query_build($query) or error(__FILE__, __LINE__);
...
else if (isset($_POST['remove']))
{
	...
	($hook = get_hook('ark_remove_form_submitted')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
	...
	($hook = get_hook('ark_qr_delete_rank')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
	$forum_db->query_build($query) or error(__FILE__, __LINE__);
...
($hook = get_hook('ark_pre_header_load')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
...
($hook = get_hook('ark_main_output_start')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null;
...
<?php ($hook = get_hook('ark_add_rank_pre_rank')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null; ?>
...
<?php ($hook = get_hook('ark_add_rank_pre_min_posts')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null; ?>
...
<?php ($hook = get_hook('ark_add_rank_end')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null; ?>
			</fieldset>
		</form>
<?php
...
<?php
	...
	?>
<?php ($hook = get_hook('ark_edit_rank_pre_rank')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null; ?>
...
<?php ($hook = get_hook('ark_edit_rank_pre_min_posts')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null; ?>
						<div class="mf-field text">
							<label for="fld<?php echo ++$forum_page['fld_count'] ?>"><span class="fld-label"><?php echo $lang_admin_ranks['Min posts label'] ?></span></label><br />
...
<?php ($hook = get_hook('ark_edit_rank_end')) ? (!defined('FORUM_USE_EVAL') ? include $hook : eval($hook)) : null; ?>
				</fieldset>
	<?php
