...
if (isset($_POST['add_forum']))
{
	...
	redirect(pun_link($pun_url['admin_forums']), $lang_admin['Forum added'].' '.$lang_admin['Redirect']);
}
...
else if (isset($_GET['del_forum']))
{
	...
	if (isset($_POST['del_forum_cancel']))
		redirect(pun_link($pun_url['admin_forums']), $lang_admin['Cancel redirect']);
	...
	if (isset($_POST['del_forum_comply']))	// Delete a forum with all posts
	{
		...
		redirect(pun_link($pun_url['admin_forums']), $lang_admin['Forum deleted'].' '.$lang_admin['Redirect']);
	}
...
<?php echo generate_admin_menu(); ?>
	...
	<div class="main-content frm">
		<div class="frm-head">
			<h2><span><?php printf($lang_admin['Confirm delete forum'], pun_htmlencode($forum_name)) ?></span></h2>
		</div>
...
else if (isset($_POST['update_positions']))
{
	...
	redirect(pun_link($pun_url['admin_forums']), $lang_admin['Forums updated'].' '.$lang_admin['Redirect']);
}
...
else if (isset($_GET['edit_forum']))
{
	...
	if (isset($_POST['save']))
	{
		...
		redirect(pun_link($pun_url['admin_forums']), $lang_admin['Forum updated'].' '.$lang_admin['Redirect']);
	}
	else if (isset($_POST['revert_perms']))
	{
		...
		redirect(pun_link($pun_url['admin_forums']).'?edit_forum='.$forum_id, $lang_admin['Permissions reverted'].' '.$lang_admin['Redirect']);
	}
				...
				<fieldset class="frm-set set<?php echo ++$pun_page['set_count'] ?>">
					...
					<div class="frm-fld text">
						<label for="fld<?php echo ++$pun_page['fld_count'] ?>">
							...
							<span class="fld-input"><input type="text" id="fld<?php echo $pun_page['fld_count'] ?>" name="forum_name" size="35" maxlength="80" value="<?php echo pun_htmlencode($cur_forum['forum_name']) ?>" /></span>
						</label>
					</div>
					<div class="frm-fld text textarea">
						<label for="fld<?php echo ++$pun_page['fld_count'] ?>">
							<span class="fld-label"><?php echo $lang_admin['Forum description'] ?></span><br />
							<span class="fld-input"><textarea id="fld<?php echo $pun_page['fld_count'] ?>" name="forum_desc" rows="3" cols="50"><?php echo pun_htmlencode($cur_forum['forum_desc']) ?></textarea></span>
...
<?php
	...
	while ($cur_cat = $pun_db->fetch_assoc($result))
	{
		...
		echo "\t\t\t\t\t\t\t\t".'<option value="'.$cur_cat['id'].'"'.$selected.'>'.pun_htmlencode($cur_cat['cat_name']).'</option>'."\n";
	}
...
?>
					<fieldset class="frm-group">
						<legend><span><?php echo pun_htmlencode($cur_perm['g_title']) ?></span></legend>
...
<?php
	...
	$result = $pun_db->query_build($query) or error(__FILE__, __LINE__);
	while ($cur_cat = $pun_db->fetch_assoc($result))
		echo "\t\t\t\t\t\t\t".'<option value="'.$cur_cat['id'].'">'.pun_htmlencode($cur_cat['cat_name']).'</option>'."\n";
...
?>
			<fieldset class="frm-set set<?php echo ++$pun_page['set_count'] ?>">
				<legend class="frm-legend"><strong><?php echo pun_htmlencode($cur_forum['cat_name']) ?></strong></legend>
				<h3 class="frm-fld link">
					<span class="fld-label"><?php echo $lang_admin['Category'] ?></span>
					<span class="fld-input">[ <?php echo pun_htmlencode($cur_forum['cat_name']) ?> ]</span>
				</h3>
<?php
...
?>
				<div class="frm-fld text twin">
					...
					<label for="fld<?php echo ++$pun_page['fld_count'] ?>" class="twin2">
						...
						<span class="fld-extra"><?php echo pun_htmlencode($cur_forum['forum_name']) ?></span>
					</label>
				</div>
<?php
