				...
				<fieldset class="frm-set set<?php echo ++$pun_page['set_count'] ?>">
					...
					<div class="frm-fld text required">
						<label for="fld<?php echo ++$pun_page['fld_count'] ?>">
							...
							<span class="fld-input"><input type="text" id="fld<?php echo $pun_page['fld_count'] ?>" name="req_title" size="25" maxlength="50" value="<?php if ($mode == 'edit') echo pun_htmlencode($group['g_title']); ?>" /></span>
					...
					<div class="frm-fld text required">
						<label for="fld<?php echo ++$pun_page['fld_count'] ?>">
							...
							<span class="fld-input"><input type="text" id="fld<?php echo $pun_page['fld_count'] ?>" name="user_title" size="25" maxlength="50" value="<?php echo pun_htmlencode($group['g_user_title']) ?>" /></span>
...
<?php if ($group['g_id'] != PUN_GUEST): ?>						<div class="radbox"><label for="fld<?php echo ++$pun_page['fld_count'] ?>"><input type="checkbox" id="fld<?php echo $pun_page['fld_count'] ?>" name="send_email" value="1"<?php if ($group['g_send_email'] == '1') echo ' checked="checked"' ?> /> <?php echo $lang_admin['Allow send email'] ?></label></div>
<?php endif; ?>					</fieldset>
				...
				<fieldset class="frm-set set<?php echo ++$pun_page['set_count'] ?>">
					...
					<div class="frm-fld text">
						<label for="fld<?php echo ++$pun_page['fld_count'] ?>">
							...
							<span class="fld-input"><input type="text" id="fld<?php echo $pun_page['fld_count'] ?>" name="email_flood" size="5" maxlength="4" value="<?php echo $group['g_email_flood'] ?>" /></span>
...
else if (isset($_POST['add_edit_group']))
{
	...
	if ($_POST['mode'] == 'add')
	{
		...
		$result = $pun_db->query_build($query) or error(__FILE__, __LINE__);
		if ($pun_db->num_rows($result))
			message(sprintf($lang_admin['Already a group message'], pun_htmlencode($title)));
		...
		$query = array(
			...
			'INTO'		=> 'groups',
			'VALUES'	=> '\''.$pun_db->escape($title).'\', '.$user_title.', '.$moderator.', '.$mod_edit_users.', '.$mod_rename_users.', '.$mod_change_passwords.', '.$mod_ban_users.', '.$read_board.', '.$view_users.', '.$post_replies.', '.$post_topics.', '.$edit_posts.', '.$delete_posts.', '.$delete_topics.', '.$set_title.', '.$search.', '.$search_users.', '.$send_email.', '.$edit_subjects_interval.', '.$post_flood.', '.$search_flood.', '.$email_flood
		);
	...
	redirect(pun_link($pun_url['admin_groups']), (($_POST['mode'] == 'edit') ? $lang_admin['Group edited'] : $lang_admin['Group added']).' '.$lang_admin['Redirect']);
}
...
else if (isset($_POST['set_default_group']))
{
	...
	redirect(pun_link($pun_url['admin_groups']), $lang_admin['Default group set'].' '.$lang_admin['Redirect']);
}
...
else if (isset($_GET['del_group']))
{
	...
	if (isset($_POST['del_group_cancel']))
		redirect(pun_link($pun_url['admin_groups']), $lang_admin['Cancel redirect']);
	...
	if (!$pun_db->num_rows($result) || isset($_POST['del_group']))
	{
		...
		redirect(pun_link($pun_url['admin_groups']), $lang_admin['Group removed'].' '.$lang_admin['Redirect']);
	}
...
<?php echo generate_admin_menu(); ?>
	...
	<div class="main-content frm">
		<div class="frm-head">
			<h2><span><?php printf($lang_admin['Remove group head'], pun_htmlencode($group_title), $num_members) ?></span></h2>
		</div>
...
?>
			<div class="grp-item databox db<?php echo ++$pun_page['item_num'] ?>">
				<h3 class="data"><span><?php echo pun_htmlencode($cur_group['g_title']) ?></span></h3>
